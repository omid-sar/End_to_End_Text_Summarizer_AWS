...
class DirectoryTree:
    ...
    def _generate_tree(self, directory: Path, level: int = 0):
        ...
        # Skip directories like '__pycache__' and '.DS_Store'
        if directory.name in ['__pycache__', '.DS_Store']:
            return
        ...
        for item in directory.iterdir():
            if item.is_dir():
                ...
                # Skip files with certain extensions, e.g., .pyc
                if not item.name.endswith(('.pyc', '.DS_Store')):
